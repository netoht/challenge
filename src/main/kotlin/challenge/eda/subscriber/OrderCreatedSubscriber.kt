...
class OrderCreatedSubscriber : Subscriber {
    ...
    private fun publishItems(orderCreated: OrderCreated, it: OrderItem) {
        ...
        logger.info("msg=sending order item, order=${orderCreated.invoice.order.id}, item=${it.product.name} type=${it.product.type}")
        PubSubService.publish(topic.name, Message(event))
    }
}
